        ...
        if ($payment_method === 'bank-transfer') {
            ...
            echo '<script>
                    document.addEventListener("DOMContentLoaded", function() { 
                        showBankTransferModal(' . json_encode($paymentData) . '); 
                    });
                  </script>';
        } else {
...
<head>
    ...
    <style>
        ...
        .buy-btn.selected {
            background-color: lightblue; /* Keep the color red when selected */
        }
            

        .buy-btn:active {
            background-color: lightblue; /* Color stays on click */
        }
...
<header>
    <div class="logo">SalesPilot - Pricing Plans</div>
    <div class="header-banner">
        <a href="sign-up.php" class="free-trial-button">Free Starter Trial - 3 Months</a>
        <span> | </span>
    ...
    <div class="container">
        <div class="row g-4">
            ...
            foreach ($planNames as $key => $planName): ?>
                <div class="col-lg-4">
                    <div class="pricing-item" data-plan="<?php echo $key; ?>">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form id="paymentProofForm" action="pay2.php" method="POST" enctype="multipart/form-data">
...
function selectPlan(plan) {
    // Set the selected plan value and update the UI
    document.getElementById('selected-plan').value = plan;
    document.getElementById('plan-summary').textContent = plan.charAt(0).toUpperCase() + plan.slice(1);
    
    // Update the total price for the selected plan
    updateTotalPrice(plan);

    // Reset background color for all plan items
    ...
    const selectedPlanItem = document.querySelector(`.pricing-item[data-plan="${plan}"]`);
    if (selectedPlanItem) {
        selectedPlanItem.style.backgroundColor = '#87CEEB'; // Light blue for selected option
        selectedPlanItem.classList.add('highlight'); // Optionally add a class for further styling
    }
}

// Function to update the total price based on the selected plan
function updateTotalPrice(selectedPlan) {
    const cycle = 'monthly';  // We are using only the monthly cycle as per your request
    ...
    window.selectedPlanPrice = totalPrice;  // Ensure this is set properly
}
...
function showBankTransferModal(paymentData) {
    // Set the Bill Amount and Total Amount dynamically
    document.getElementById("bankAmountDisplay").textContent = "$" + paymentData.amount;
    document.getElementById("totalAmountDisplay").textContent = "$" + paymentData.amount;  // You can also calculate NGN if needed

    // Set order_id dynamically
...
// Listen for when the form is submitted for payment proof
